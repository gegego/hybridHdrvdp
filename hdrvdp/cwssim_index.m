function [cwssim,map] = cwssim_index(img1, img2, level, or, guardb, K)
%========================================================================
...
map = zeros(size(img1,1), size(img2,2), 'double');                                                                        % finest level of pyramid decomposition.
...
for i=1:or
   ...
   corr_band = filter2(window, corr, 'same');%.........% The CW-SSIM indices are computed locally using a sliding 7-by-7 
   varr_band = filter2(window, varr, 'same');%.........% window that moves across each wavelet subband.
